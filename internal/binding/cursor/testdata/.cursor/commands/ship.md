1. Act as the `build` agent using the instructions in the `build` rule.
...
5. Repeat the following steps while `code_review.review` equals `revise`, at most `3` times (advisory — track your iteration count and stop at 3):
  - Act as the `build` agent using the instructions in the `build` rule using the output from `code_review.reviewer`.
  ...
  - Act as the `reviewer` agent using the instructions in the `reviewer` rule using the output from `code_review.loop.build`. Read the last `agentflow-output` block. If it is missing or invalid, re-invoke the agent up to `0` times, then stop the flow. Allowed values: approve, revise, reject.
